...
async function load() {
  ...
  for (const key of ["transport_mode", "gateway_endpoint", "model_mode", "model_endpoint", "dispatch_poll_seconds", "observation_seconds", "approval_policy_mode"]) $(key).value = config[key];
  $("fixture_test_mode").checked = Boolean(config.fixture_test_mode);
}
...
$("config").onsubmit = async (event) => {
  ...
  status(await request({ type: "BHR_SAVE_CONFIG", patch: {
    ...
    fixture_test_mode: $("fixture_test_mode").checked,
    approval_policy_mode: $("approval_policy_mode").value,
