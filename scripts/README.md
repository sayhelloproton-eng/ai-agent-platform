...
不会删除其他进程已存在的状态文件。成功 `wx` 创建后才记录本实例所有权；
`runBridge` 的最外层退出路径始终再次尝试删除本实例拥有的状态文件，不依赖
子进程清理是否成功或最终返回码，且删除动作幂等。
...
进程失败不会阻止其他进程清理；最外层状态清理仍独立执行，也不会无限等待。
状态删除失败会形成稳定清理错误，但不会阻止受管子进程的清理。端口占用时
立即失败，不杀进程、不接管服务、不更换端口。
