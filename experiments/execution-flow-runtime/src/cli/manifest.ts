...
export function getCliManifest() {
  const commands: CliCommandDescriptor[] = [
    { command: "install", purpose: "Initialize runtime home and default local config. It does not install the npm package.", usage: "aap-execution-flow install [--json]", side_effect: "filesystem", machine_output: true },
    { command: "config", purpose: "Read or persist runtime-owned provider configuration. Provider settings are stored in runtime config, not shell aliases or per-command environment variables.", usage: "aap-execution-flow config [show | mlxhub set | mlxhub clear] [options] [--json]", side_effect: "filesystem", machine_output: true },
    { command: "start", purpose: "Start the single managed local HTTP service in the background. Idempotent for the current runtime home.", usage: "aap-execution-flow start [--json]", side_effect: "process", machine_output: true },
    { command: "serve", purpose: "Run the HTTP service in the foreground.", usage: "aap-execution-flow serve", side_effect: "process", machine_output: false },
    ...
    { command: "restart", purpose: "Stop then start the local service.", usage: "aap-execution-flow restart [--json]", side_effect: "process", machine_output: true },
    ...
    { command: "spec", purpose: "List or print a public JSON Schema bundled with the module.", usage: "aap-execution-flow spec [list|template-value|execution-run|execution-flow|execution-result|capability|inference-node] [--json]", side_effect: "none", machine_output: true },
