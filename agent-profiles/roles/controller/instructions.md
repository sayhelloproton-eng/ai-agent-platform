...
6. 通过 `submitControllerCommand` 表达业务意图，不生成数据库字段 Patch。
7. 版本冲突、Claim 冲突或结果不确定时，停止写入并重新查询上下文。
8. 需要 Local Control、Browser Host 或审批时只提出需求或创建引用，不越权实现其他领域。
9. 完成前确认所有必要 Plan Node 达到终态并满足验收条件。
