...
## ALIGN-001～250 增量修复：Browser / Worker / Execution 边界

原文中的 Phase2 `Browser Host / Execution Lease` 等继续仅作为 historical evidence。Phase3 normative boundary 统一为：Agent owns Worker identity；Task owns TaskRoleBinding/workflow；Execution Browser owns CREATE/RESTORE/WAKE/page state/permission/screenshot/delivery/effect/recovery。Browser 不再直接推进 Task 状态，只通过 Task Public API 写回。

<!-- OPENAI-CARRIER-ABSORPTION-20260812 -->

# 11. Custom GPT Worker 的 Context Delivery

Task 仍只提供自己的正式 context：

```text
Task / Node / run / TaskRoleBinding
TaskDocument metadata/content
TaskMessage/Event refs
```

Custom GPT Worker 的传输策略改为：

```text
WAKE → 小型 task/node/worker trigger
getNodeContext → 小型结构化事实 + document metadata
getTaskDocument → Gateway → openaiFileResponse
```

因此 Browser 不再复制 Task Markdown 全文；Task 也不为 Carrier 新增一套“Conversation Context”存储。OpenAI file id/download URL 只是 transport，不属于 Task state。
...
## OpenAI Carrier 文件桥对 Task 的最小影响

Task 不新增 File/Artifact/Context Domain。`getNodeContext` 默认返回小型 Task/Node facts + TaskDocument metadata；Custom GPT 需要正文时由 Gateway 把 TaskDocument 适配为 `openaiFileResponse`。GPT 提交文件时，Carrier adapter 先把 `openaiFileIdRefs` 转为 canonical TaskDocument 输入。OpenAI file id/download_link 只属于 transport provenance，不进入 Task 主键或 durable locator。
