...
Task 已存在，但前置条件尚未满足。可存在 taskId、title / objective、Plan / Nodes、Requirement 等初始文档，但不能开始、不产生可执行 Node、不绑定 Worker。

### READY

Task 已满足前置条件，可以开始。

独立 Task：`READY → ACTIVE` 需要人工点击执行。

属于已人工激活的 TaskGroup：后续符合条件的 Task 可以由扩展 / 自动化调用 `startTask`，但 Task Domain 仍是唯一合法性校验者。
...
表示技术 / 系统执行失败，例如 Browser Host unavailable、本地服务失败、文件无法写入、Worker 执行异常。
...
Browser Host 根本无法运行、测试脚本未执行、环境损坏等才属于 Node FAILED。
...
Task Domain 不选择 Worker、不维护 Worker Pool、不理解 Conversation URL、不做重新分配。

运行中 `workerRef` 不可修改。若需换 Worker：`reopen Node → runNo + 1 → workerRef = null → READY`，再重新选择。
...
独立 Task 通常创建为 READY；属于 TaskGroup 且前序未完成时为 PENDING。
...
调用前 Agent Domain 先选择具体 `workerRef`。
...
```text
Role = 资格 / 角色要求
Worker = 具体执行实例 / Session / Conversation
```
