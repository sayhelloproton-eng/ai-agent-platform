...
> **2026-08-12 POST-ALIGNMENT SUPERSESSION RULE**：本文件是高密度聚合真源，内部保留了此前各专题全文。凡出现“Browser Extension / Dev Tunnel 属于 Agent Domain”“七个 Agent 独立单元”“Extension 负责 Task binding 真源”等旧 ownership 表述，均保留为历史设计上下文，并由 ALIGN-008/005/016/029/073～079 的最终补丁覆盖：Browser→Execution；Dev Tunnel→Deployment External Resource；TaskRoleBinding→Task；Agent→Role/Worker identity + logical Collaboration。
...
> **智能体运行与协作领域负责 Agent Package、部署后的 Role/认证、具体 Worker 的 Carrier 身份语义、Custom GPT 公网 Gateway 与跨角色 Collaboration。**Browser Extension 的唯一领域 Owner 是 Execution；Dev Tunnel 是 Deployment External Resource Module。** Task Workflow 与真实执行副作用仍由对应领域拥有。**
...
# Superseded by ALIGN-008 / ALIGN-029:
# browser-extension → Execution Domain Deployment Unit
# dev-tunnel → Deployment External Resource Module
```
...
产品 Worker 例外只在“创建时机”：它在 Task 前由用户主动打开新的产品 GPT Conversation 并完成需求沟通；该 Conversation 在 createTask 时成为正式 product worker。研发/测试 Worker 则在 Task human authorization 后，由 Execution Browser provisioning flow 真实 CREATE/识别 Conversation，Agent 校验 Worker identity，Task `bindTaskWorker` 固化 TaskRoleBinding。
...
**Historical v0.2 statement（已 superseded）：** 当时“七个独立单元”曾被确认；当前 Browser→Execution、Dev Tunnel→Deployment External Resource，Agent 自有五个 packages。
...
**Historical v0.2 lifecycle statement（已部分 superseded）：** 原七单元曾按统一 lifecycle 描述；当前每个 Module 只暴露其真实 kind/lifecycle，不伪造 lifecycle。
...
## ALIGN-001～250 增量修复：恢复与真实 Carrier E2E
...
→ 保留原 TaskDocument/Execution Artifact 真源
→ 重新生成短期 relay
...
- 建立最小 Task/Role/Worker identity + trigger context；大型动态文档改由 GPT Actions File Bridge 按需传输；
...
三个角色全部通过；进入 dogfood 前必须验证**至少一种可靠的 Product `workerRef/c-id` 获取路径**。`getCurrentCarrierContext/current-link` 仅作为 `PENDING_SPIKE` 候选路径，不得作为 mandatory gate；若目标 Custom GPT 环境不能稳定提供 current-link/c-id，则使用 Execution Browser / Carrier observation 从真实页面 URL 取得并校验 `g-id + c-id`。只要该 Browser observation E2E PASS，即可进入 dogfood；不得因 current-link Spike 失败阻断实施。
...
**Historical v0.2 statement（已部分 superseded）：** 当时方案要求以下七个单元独立管理；当前 ownership 以本聚合文件顶部 POST-ALIGNMENT SUPERSESSION RULE 为准：
...
Microsoft Dev Tunnels 适配能力仍必须 Module 化；最终 Owner 为 Deployment External Resource Module（ALIGN-029）。
...
**Historical v0.2 statement（已部分 superseded）：** 原七单元版本治理描述保留；当前实际 Module ownership 以 ALIGN 后 package/module graph 为准。
...
## ALIGN-001～250 总纲对齐增量修复（Post-freeze patch）

本节只修复跨领域 ownership / package / runtime topology 冲突，不替换此前高密度 Agent 设计。

- Agent owns：Agent Package、Role Registry、Role credential binding semantics、Worker/Conversation identity、Collaboration logical facts、Agent Gateway。
- Task owns：TaskRoleBinding（Task 内 roleRef→workerRef 稳定关系）与 workflow。Agent 不保存第二份 Task→Worker binding 真源。
- Execution owns：Browser Extension、真实 CREATE/RESTORE/WAKE、DOM/page state、permission、peer message physical delivery、browser effect/evidence/UNKNOWN。
- Deployment owns：Dev Tunnel 作为 External Resource Module；Agent 不再把 tunnel/browser-extension 计入自己的发布/部署单元清单。
- `agent-runtime` 是独立 npm package，但 v1 由 `@ai-agent-platform/platform-host` in-process 装配；`agent-gateway` 保持独立公网 Service/Process/Deployment Unit。
- Gateway 是 OpenAI Carrier Anti-Corruption Layer，只做 auth/basic validation/protocol normalization/routing/result normalization，不成为 Task/Execution/Model 第二真源。
- OpenAI File Bridge 的官方协议字段/硬限制已进入 Carrier/Gateway normative contract；Always Allow、Multi-Action Turn、Conversation-native file search、Code Interpreter Context Pack/Patch 的目标环境稳定性继续按 `PENDING_SPIKE` 验证。

<!-- OPENAI-CARRIER-ABSORPTION-20260812 -->

## OpenAI Carrier 能力吸收后的权威增量规则

本节覆盖本聚合文档中更早的 Carrier transport 实现描述，但不替换历史上下文：

```text
small structured control → GPT Action JSON
documents/files/large context → openaiFileIdRefs / openaiFileResponse
Conversation CREATE/RESTORE/WAKE/c-id → Execution Browser
page state/screenshot → Execution Browser + Vision
real local/browser effect → Execution
```

Gateway 必须处理：File Bridge runtime shape、45s/100k/TLS/443、真实 429/5xx、Custom Header 禁止、每 operation 显式 `x-openai-isConsequential`。Routine Action 的 Carrier confirmation 不替代 Execution Approval。

Browser 大型 TaskContext DOM 注入、每 Action 一次额外 WAKE、把 routine permission 自动点击当作正常必经步骤，从目标 happy path 裁掉；Always Allow / Multi-Action / Conversation file search / Context Pack 的真实稳定性在 E2E 后把对应 fallback 进一步收窄。 Multi-Action Spike FAIL 时回退为 bounded multiple Worker Turns，每 Turn 先重读 owner facts 且不重放已成功 Action/Effect；current-link 也不是 dogfood mandatory gate，可靠 Browser/Carrier c-id observation 可满足身份门。 File Bridge 的 P0 size/fetch/relay/serializer/error/security budgets 以本文件 13.5 与 Deployment Conformance 为准。
