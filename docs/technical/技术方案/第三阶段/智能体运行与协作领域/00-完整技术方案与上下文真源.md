...
**Superseded by ALIGN-008/029：** 原七单元 ownership 已拆分，由对应 Owner 分别进入统一 Module Governance。
...
- OpenAI File Bridge / Always Allow / Multi-Action Turn / Code Interpreter Context Pack 等新发现进入 `VERIFIED / PENDING_SPIKE / REJECTED_OR_UNSUPPORTED`，未验证前不得改写主业务合同。
