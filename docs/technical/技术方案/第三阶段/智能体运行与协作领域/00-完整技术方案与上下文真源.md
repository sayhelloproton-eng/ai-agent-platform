...
→ 不持久化旧 URL
→ 重新发起当前 Worker Action / 重新取得 file ref
→ 不盲用旧 bytes
...
Browser 大型 TaskContext DOM 注入、每 Action 一次额外 WAKE、把 routine permission 自动点击当作正常必经步骤，从目标 happy path 裁掉；Always Allow / Multi-Action / Conversation file search / Context Pack 的真实稳定性在 E2E 后把对应 fallback 进一步收窄。
