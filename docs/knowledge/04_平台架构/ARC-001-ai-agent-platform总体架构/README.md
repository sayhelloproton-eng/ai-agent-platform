...
用户 / Agent / 客户端
        ↓
1. Agent Interface：Chat、Web、CLI、IDE、Mobile
        ↓
2. Agent Brain：理解、规划、推理、角色与策略
        ↓
3. Agent Runtime：Task Control、状态、执行、恢复
        ↓
4. Tool Layer：Actions、MCP、API、Skills、Adapters
        ↓
5. Knowledge Layer：Context、Registry、Knowledge、Evidence
        ↓
6. Infrastructure：Git、Cloud、Local、Network、Storage
```

横切所有层：身份与认证、Policy 与审批、可观测性与成本、版本与审计。当前已验证的是知识治理和 Custom GPT → Dev Tunnels → Gateway → Runtime 的窄链路；动态 Task Control、多执行器编排和业务产品仍是目标设计。

- Visual Asset ID：`VIS-001`；
- 可编辑源文件：[`./assets/VIS-001-平台六层总体架构.svg`](./assets/VIS-001-平台六层总体架构.svg)；
- 人类预览：[`./assets/VIS-001-平台六层总体架构.png`](./assets/VIS-001-平台六层总体架构.png)；
