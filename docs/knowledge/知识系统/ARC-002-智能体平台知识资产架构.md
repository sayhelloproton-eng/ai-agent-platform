...
canonical_path: docs/knowledge/知识系统/ARC-002-智能体平台知识资产架构.md
feishu:
  mode: projection
  logical_path: 知识系统/智能体平台知识资产架构
related_assets:
  - ADR-002
  - SKL-001
  - CTX-001
...
由 Git 生成的阅读镜像、汇总页、索引页、项目首页、状态看板、ADR 索引和作品集。它不是独立事实源。
...
关系索引位于 `docs/technical/元数据/relations.yaml`。
...
| `capture` | 临时收集，等待分类 | 捕获收件箱（Capture Inbox） |
...
任务
  ↓
读取 docs/technical/元数据/assets.yaml
  ↓
按类型、关系、状态和证据筛选
  ↓
读取最多量必要正文
...
2. 下一阶段：生成 Feishu 对齐 Write Plan，并实现单资产 Git → Feishu Projection MVP。
3. 后续阶段：加入 Drift 调度、Runtime Evidence 外部存储和跨 Provider 检索。
